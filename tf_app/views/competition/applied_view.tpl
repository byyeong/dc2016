                ...
                <div  class="row"> 
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-9 content">
                        ...
                        <div class="content-header">
                            <h3 class="content-heading"><span class="icon-trophy"></span> 경진 대회 참가자 내용보기</h3> 
                        </div>
                        ...
                        <!--
                        {? examiner || res.user_id == s.id} 
                        <ul class="pager">
                            <li {? !prev}class="disabled"{/}>{? prev}<a href="/competition/applied_view/{prev.id}">&larr; 이전</a>{:}<a href="#" onclick="return false;">&larr; 이전</a>{/}</li>
                            <li {? !next}class="disabled"{/}>{? next}<a href="/competition/applied_view/{next.id}">다음 &rarr;</a>{:}<a href="#" onclick="return false;">다음 &rarr;</a>{/}</li>
                        </ul> 
                        {/} 
                        -->
                    </div>
    ...
    function get_ex_list(_id, _type, _page, _div) {
        var get_url = "/ajax_front/get_ex_list_all/"+_id+"/"+_type+"/"+_page+"/"+_div;
...
function get_apply(_this, _id) {
    ...
    $.get("/ajax_front/get_apply/"+_id, function(_data){
...
function add_scoring()
{
    ...
    $.ajax({
        ...
        success : function(_data) {
            ...
            window.location.href = "/competition/applied_view/"+$('input[name=apply_id]').val();
        }
    });
}
...
function change_status()
{
    ...
    $.ajax({
        ...
        success : function(_data) {
            ...
            window.location.href = "/competition/applied_view/"+$('input[name=id]').val();
            // get_ap_list('{res.id}', 'all', 1, 'applied-all');
            // get_ap_list('{res.id}', 'suc', 1, 'applied-received');
            // get_ap_list('{res.id}', 'pre', 1, 'applied-standby');
            // get_ap_list('{res.id}', 'rej', 1, 'applied-rejected');
