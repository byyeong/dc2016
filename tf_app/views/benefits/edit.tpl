...
{? msgt}
{? res.status == 1}
_m = "Benefits(스타트업 지원 프로그램) 상세 설정을 완료했습니다.<br><br>저장 후 신청 지원서 작성을 시작합니다.";
$('#md-warning-ch .btn-danger').html('다음으로');
$('#md-warning-ch .btn-danger').attr('onclick', 'javascript:location.href="/benefits/apply_info/{res.id}"');
$('#md-warning-ch p').html(_m);
$('#md-warning-ch').modal('show');
{:}
_m = "Benefits(스타트업 지원 프로그램) 상세 설정을 완료했습니다.<br><br>저장 후 목록로 이동합니다.";
$('#md-warning-ch .btn-danger').html('목록으로');
$('#md-warning-ch .btn-danger').attr('onclick', 'javascript:location.href="/member/edit_benefit_m"');
$('#md-warning-ch p').html(_m);
$('#md-warning-ch').modal('show');
{/}
{/}
            ...
            <div class="container">
                <div class="editor">
                    <div class="edit-header">
                        <h3><a href="/member/edit_benefit_m">Benefits 관리</a> > 프로그램 상세 설정</h3>
                     </div>
                    <div class="edit-body">   
                        <!-- tab-content -->
                        <ul class="nav nav-tabs">
                            <li class="active"><a href="/benefits/edit/{res.id}">{? res}<span class="icon-ok-sign done"></span>{:}<span class="icon-ok todo"></span>{/}신청 정보 입력</a></li>
                            {? res.app}
                            <li><a href="/benefits/apply_info/{res.id}">{? res.basic || res.forms || res.form_file}<span class="icon-ok-sign done"></span>{:}<span class="icon-ok todo"></span>{/}지원서 만들기</a></li>
                            {/}
                        </ul>
                        <div class="tab-content">
                            <div class="tab-pane active" id="formInfo">
                                <form class="form-horizontal" name="edit_pro_f" action="/benefits/edit/{res.id}" role="form" method="post" onsubmit="return false;" enctype="multipart/form-data" >
                                    <fieldset>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-12 col-sm-9 col-sm-9">
                                                <label class="file-inline">
                                                    {? res.file}<a href="/benefits/get_pr_source?ev={res.id}">{res.filename}</a> <input type="checkbox" name="del_file" value="{res.file}"> 파일 삭제{/}
