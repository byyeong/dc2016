        ...
        <div class="page-header">
            <div class="container">
                ...
                <div class="pull-right">
                    ...
                    {:}
                    <a href="#" class="btn btn-primary pull-right" onclick="javascript:alert('로그인이 필요합니다.');">
                        제품/서비스 등록하기 
                    </a>
                    {/}
                </div>
                            ...
                            <li class="media {.id}"> 
                                ...
                                <div class="col-xs-12 {? !.img1}col-sm-12 col-md-12{:}col-sm-7 col-md-7{/} media-body ">
                                    <div class="product-status text-right">
                                        ...
                                        {? .diff > 0 && .diff < 5}<span class="label label-success ">{.diff * 24}시간 전</span>{/}
                                        {? .diff > 0 && .diff > 4}<span class="label label-success ">{.diff}일 전 </span>{/}
                                        {? .diff < 0}<span class="label label-success ">{=abs(.diff)}일 후 </span>{/}
                                    </div>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-3 sidebar">
                        <section class="list-category">
                            ...
                            <ul class="list-unstyled">
                                <!--li><a href="/showcase">All <span class="cnt">({tot_a})</span></a></li-->
                                <li><a href="/showcase?status=just_launched">{? status == 'just_launched'}<b>{/}Just Launched! <span class="cnt">({tot_j})</span>{? status == 'just_launched'}</b>{/}</a></li>
                                <!--li><a href="/showcase?status=archived">서비스중 <span class="cnt">({tot_a})</span></a></li-->
                                <li><a href="/showcase?status=ready">{? status == 'ready'}<b>{/}개발중 <span class="cnt">({tot_r})</span>{? status == 'ready'}</b>{/}</a></li>
                            </ul>
                        </section>
                    </div>
                    <!-- /sidebar -->
