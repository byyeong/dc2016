...
class Space extends Controller
{
    ...
    public function index($_date = '', $_next = '')
    {
        ...
        for ($dd=0; $dd<7; $dd++) {
            for ($e=0; $e<$len_sp2; $e++) {
                $event = $this->Space_reserve->get_by_date_o_space("space_id = ".$sp2[$e]["id"]." and date = '".$res[$dd]["date"]."' and state = 1");
                $ev2[$dd]["space"][$e]["event"] = $event;
                $ev2[$dd]["space"][$e]["name"] = $event[0]["sname"];
                $ev2[$dd]["space"][$e]["user"] = $this->Members->get($event[0]["user_id"]);
            }
        }
        ...
        for ($d=0; $d<7; $d++) {
            for ($e=0; $e<$len_sp4; $e++) {
                $event = $this->Space_reserve->get_by_date_o_space("space_id = ".$sp4[$e]["id"]." and date = '".$res[$d]["date"]."' and state = 1");
                $ev4[$d]["space"][$e]["event"] = $event;
                $ev4[$d]["space"][$e]["name"] = $event[0]["sname"];
                $ev4[$d]["space"][$e]["user"] = $this->Members->get($event[0]["user_id"]);
            }
        }
        ...
        for ($f=0; $f<7; $f++) {
            for ($e=0; $e<$len_sp5; $e++) {
                $event = $this->Space_reserve->get_by_date_o_space("space_id = ".$sp5[$e]["id"]." and date = '".$res[$f]["date"]."' and state = 1");
                $ev5[$f]["space"][$e]["event"] = $event;
                $ev5[$f]["space"][$e]["name"] = $event[0]["sname"];
                $ev5[$f]["space"][$e]["user"] = $this->Members->get($event[0]["user_id"]);
            }
        }
        ...
        for ($g=0; $g<7; $g++) {
            for ($e=0; $e<$len_sp6; $e++) {
                $event = $this->Space_reserve->get_by_date_o_space("space_id = ".$sp6[$e]["id"]." and date = '".$res[$g]["date"]."' and state = 1");
                $ev6[$g]["space"][$e]["event"] = $event;
                $ev6[$g]["space"][$e]["name"] = $event[0]["sname"];
                $ev6[$g]["space"][$e]["user"] = $this->Members->get($event[0]["user_id"]);
            }
        }
