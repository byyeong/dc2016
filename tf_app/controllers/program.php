...
class Program extends Controller
{
    ...
    var $use_models = array("Program", "Startup_member", "Program_form", "Program_file"
            , "Program_apply", "Startup_service", "Members");
    ...
    {
        header("Location: /benefit/views/".$_id);
    ...
    {
        header("Location: /benefit/views/".$_id.'/'.$_per);
        $r = $this->Program->get_list('', '', 'c.id = '.$_id);
        $res = $r[0];
        if (!$res) 
            header("Location: /program");
    ...
    {
        header("Location: /benefit/apply_info/".$_id);
        if (!$_SESSION["s"]) {
            $_SESSION["msg"] = "로그인 후 이용하실 수 있습니다.";
            header("Location: /");
        }
    ...
    {
        header("Location: /benefit/applied_view/".$_id);
        if (!$_SESSION["s"]) {
            $_SESSION["msg"] = "로그인 후 이용하실 수 있습니다.";
            header("Location: /");
        }
    ...
    {
        header("Location: /benefit/applied_excel/".$_id);
        ...
        for($i=0; $i<$len_res; $i++) {
            ...
            for ($j=0; $j<sizeof($file); $j++) {
                if ($file[$j]["file"]) echo "<td>".$this->settings->main_service_url."/benefit/get_source/?file=".$file[$j]["file"]."</td>";
            }
            echo "</tr>";
        }
    ...
    {
        header("Location: /benefit/get_source/");
        if (!$_SESSION["s"]) {
            $_SESSION["msg"] = "로그인 후 이용하실 수 있습니다.";
            header("Location: /");
        }
        ...
        // header("Pragma: no-cachttp://localhost.me:8006/benefit/get_pr_source?ev=2&file=0226_%EB%A1%9C%EA%B7%B8%EC%9D%B8%20%EC%83%81%ED%83%9C%20%EC%9C%A0%EC%A7%80.JPGhe"); 
        header("Expires: 0"); 
        } 
    ...
    {
        header("Location: /benefit/get_pr_source/");
        $this->tpl_name = "";
        $cp = $this->Program->get($_REQUEST["ev"]);

        $filename = trim($cp["file"]); 
        $file = "./media/benefit/$filename"; 
        // echo $file;exit(1);
        $file_size = filesize($file); 
        $filename = urlencode("$filename");
