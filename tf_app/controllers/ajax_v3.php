...
class Ajax_v3 extends Controller
{
    ...
    var $use_models = array("Competitions_form", "Competitions_file", "Competitions"
            ...
            , "Competitions_score", "Content", "Recommend", "Recruit", "Recruit_opt", "Banner", "Activity"
...
// challenge
    ...
    public function comp_form_reorder($_id)
    {
        $this->tpl_name = '';
        $forms = $this->Competitions_form->get_by_comp($_id);
        if(array_key_exists('form_list',$_REQUEST)){
            $form_list = $_REQUEST['form_list'];
            //순서 변경 및 업데이트
            foreach($form_list as $key=>$value){
                $temp = search($forms, "id", $value);
                $temp = $temp[0];
                $temp['order_no'] = $key;
                $result = $this->Competitions_form->add($temp);
                printr($result);
            }
            return true;
        }else{
...
// Showcase
    ...
    public function more_betazone_n($_page)
    {
    ...
    public function get_service($_id) 
    {
        ...
        for ($r=0; $r<sizeof($res); $r++) {
            $date = $res[$r]["date"];
            if ($_type != 'startup') $res[$r] = $this->Startup_service->get($res[$r]["service_id"]);
            if ($_type == 'startup' || ($_type != 'startup' && $res[$r]["status_bz"] == 1)) {
                $res[$r]["cats"] = $this->Startup_service->get_category($res[$r]["id"]);
                $res[$r]["s"] = $this->Startup->get($res[$r]["startup_id"]);
                $res[$r]["com"] = $this->Board->cnt("p_type = 9 and p_id = ".$res[$r]["id"]);
    ...
    public function dashboard_monthly($year=null, $month=null){
        if(is_null($year)){
            $year = date("Y");
            $month = date("m");
            $year_month2 = "DATE(NOW())";
        }
        else
        {
            $year_month2 = "LAST_DAY('".$year."-".$month."-01')";
        }
        $year2 = $year - 1;
        ...
        //승인된 전체 회원
        $user["authorized"] = $this->Members->cnt("state = 1 AND DATE(date_created) <= ".$year_month2);
        //해당월 신규회원
        $user["new"] = $this->Members->cnt('MONTH(date_created ) = '.$month.' AND YEAR(date_created) = '.$year);
        //이전월 신규회원
        $user["prev_new"] = $this->Members->cnt('MONTH(date_created ) = '.$prev_month.' AND YEAR(date_created) = '.$prev_year);
        //정리 및 탈퇴 회원
        $user["withdraw"] = $this->Members->cnt("name = 'q' AND (MONTH(date_edited) = ".$month." AND YEAR(date_edited) = ".$year.") 
                OR (state_des = '장기휴면 계정' and MONTH(date_edited ) = ".$month." AND YEAR(date_edited) = ".$year2.")");
        //회원 증감율
        $user['growth'] = round(( $user["prev_new"] == 0 ) ? '' : (($user["new"] - $user["prev_new"]) / $user["prev_new"] ) * 100);

        //승인된 전체 멤버십
        $member["authorized"] = $this->Recommend->cnt("r.state = 1 and m.state = 1 AND DATE(r.date_created) <= ".$year_month2);
        //해당월 신규멤버십
        $member["new"] = $this->Recommend->cnt('r.state = 1 and m.state = 1 AND MONTH(r.date_created ) = '.$month.' AND YEAR(r.date_created) = '.$year);
        //이전월 신규멤버십
        $member["prev_new"] = $this->Recommend->cnt('r.state = 1 and m.state = 1 AND MONTH(r.date_created ) = '.$prev_month.' AND YEAR(r.date_created) = '.$prev_year);
        //멤버십 증감율
        $member['growth'] = round(( $member["prev_new"] == 0 ) ? '' : (($member["new"] - $member["prev_new"]) / $member["prev_new"] ) * 100);
        //정리 및 탈퇴 멤버십
        $member["withdraw"] = $this->Recommend->cnt("m.name = 'q' AND ((MONTH(m.date_edited) = ".$month." AND YEAR(m.date_edited) = ".$year.") 
                OR (m.state_des = '장기휴면 계정' AND MONTH(m.date_edited ) = ".$month." AND YEAR(m.date_edited) = ".$year2."))");
        //승인된 전체 스타트업
        $startup["authorized"] = $this->Startup->cnt("state = 1 AND DATE(date_created) <= ".$year_month2);
        //해당월 신규스타트업
        $startup["new"] = $this->Startup->cnt('MONTH(date_created ) = '.$month.' AND YEAR(date_created) = '.$year);
        //이전월 신규스타트업
        $startup["prev_new"] = $this->Startup->cnt('MONTH(date_created ) = '.$prev_month.' AND YEAR(date_created) = '.$prev_year);
        //스타트업 증감율
        $startup['growth'] = round(( $startup["prev_new"] == 0 ) ? '' : (($startup["new"] - $startup["prev_new"]) / $startup["prev_new"] ) * 100);

        //승인된 전체 이벤트
        $event["authorized"] = $this->Event->cnt("state = 1 AND DATE(date_created) <= ".$year_month2);
        //해당월 신규 이벤트
        $event["new"] = $this->Event->cnt('MONTH(date_created ) = '.$month.' AND YEAR(date_created) = '.$year);
        //이전월 신규 이벤트
        $event["prev_new"] = $this->Event->cnt('MONTH(date_created ) = '.$prev_month.' AND YEAR(date_created) = '.$prev_year);
        //이벤트 증감율
        $event['growth'] = round(( $event["prev_new"] == 0 ) ? '' : (($event["new"] - $event["prev_new"]) / $event["prev_new"] ) * 100);

        //승인된 전체 블로그
        $blog["authorized"] = $this->Content->cnt("c.state = 1 AND DATE(c.date_created) <= ".$year_month2);
        //해당월 신규 블로그
        $blog["new"] = $this->Content->cnt('MONTH(c.date_created ) = '.$month.' AND YEAR(c.date_created) = '.$year);
        //이전월 신규 블로그
        $blog["prev_new"] = $this->Content->cnt('MONTH(c.date_created ) = '.$prev_month.' AND YEAR(c.date_created) = '.$prev_year);
        //블로그 증감율
        $blog['growth'] = round(( $blog["prev_new"] == 0 ) ? '' : (($blog["new"] - $blog["prev_new"]) / $blog["prev_new"] ) * 100);


        //승인된 전체 채용정보
        $recruit["authorized"] = $this->Recruit->cnt("", "state = 1 AND DATE(date_created) <= ".$year_month2);
        //해당월 신규 채용정보
        $recruit["new"] = $this->Recruit->cnt("", 'MONTH(date_created ) = '.$month.' AND YEAR(date_created) = '.$year);
        //이전월 신규 채용정보
        $recruit["prev_new"] = $this->Recruit->cnt("", 'MONTH(date_created ) = '.$prev_month.' AND YEAR(date_created) = '.$prev_year);
        //채용정보 증감율
        $recruit['growth'] = round(( $recruit["prev_new"] == 0 ) ? '' : (($recruit["new"] - $recruit["prev_new"]) / $recruit["prev_new"] ) * 100);

        //승인된 전체 공간대여
        $space["authorized"] = $this->Space_reserve->cnt("sr.state = 1 AND DATE(sr.date_created) <= ".$year_month2);
        //해당월 신규 채용정보
        $space["new"] = $this->Space_reserve->cnt('MONTH(sr.date_created ) = '.$month.' AND YEAR(sr.date_created) = '.$year);
        //이전월 신규 채용정보
        $space["prev_new"] = $this->Space_reserve->cnt('MONTH(sr.date_created ) = '.$prev_month.' AND YEAR(sr.date_created) = '.$prev_year);
        //채용정보 증감율
        $space['growth'] = round(( $space["prev_new"] == 0 ) ? '' : (($space["new"] - $space["prev_new"]) / $space["prev_new"] ) * 100);

        $this->assigns["user"] = $user;
        $this->assigns["member"] = $member;
        $this->assigns["startup"] = $startup;
        $this->assigns["event"] = $event;
        $this->assigns["blog"] = $blog;
        $this->assigns["recruit"] = $recruit;
        $this->assigns["space"] = $space;
    }
...
// 2.
    ...
    public function oauth2callback(){
        ...
        $client->refreshToken('1/Es2KIT8aRAru2gg7zYcCZJnFD2iuCQ-Xi1Nedjd8TAU');
        // ya29.xQG50wkqyJ9SHTLCKeCXAkA9WMgUHncmwTRNeYW7pXiEEBB7UJw_2ZFduWwumv_x_zu1
        $_SESSION['token']=$client->getAccessToken();
        echo "-----";
        }
